...
env $build_env ./x.py build --target aarch64-unknown-nto-qnx710 --target x86_64-pc-nto-qnx710 --target x86_64-unknown-linux-gnu rustc library/core library/alloc/
```
...
exclude_tests=
exclude_tests="$exclude_tests --exclude src/bootstrap" # suggested to be ignored by src/ci/docker/host-x86_64/i686-gnu/Dockerfile
exclude_tests="$exclude_tests --exclude src/test/rustdoc-js" # suggested to be ignored by src/ci/docker/host-x86_64/i686-gnu/Dockerfile
exclude_tests="$exclude_tests --exclude src/tools/error_index_generator" # suggested to be ignored by src/ci/docker/host-x86_64/i686-gnu/Dockerfile
exclude_tests="$exclude_tests --exclude src/tools/linkchecker" # suggested to be ignored by src/ci/docker/host-x86_64/i686-gnu/Dockerfile
exclude_tests="$exclude_tests --exclude tests/ui-fulldeps" # tests run only on host (https://rust-lang.zulipchat.com/#narrow/stream/182449-t-compiler.2Fhelp/topic/Running.20tests.20on.20remote.20target)
exclude_tests="$exclude_tests --exclude tests/codegen"
exclude_tests="$exclude_tests --exclude rustc"
exclude_tests="$exclude_tests --exclude rustdoc"
exclude_tests="$exclude_tests --exclude src/test/mir-opt"
exclude_tests="$exclude_tests --exclude tests/run-make-fulldeps" # also excluded for e.g. Apple, see .github/workflows/ci.yml

env $build_env ./x.py test $exclude_tests --stage 1 --target x86_64-pc-nto-qnx710
```
